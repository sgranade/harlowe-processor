...
macro_name_pattern = r'((?P<name>[\w\-{0}\\/][\w\-{0}]*)|(?P<variable>{1})):'.format(unicode_letters_pattern, variable_pattern)
...
def parse_link(match, s):
	...
	if contents:
		...
		if s1:
			raise RuntimeError('Parsing a link\'s description failed with "{}" text left over (full description: "{}")'.format(s1, description))
		tokenized_dest, _, s1 = tokenize(destination)
		if s1:
			raise RuntimeError('Parsing a link\'s destination failed with "{}" text left over (full destination: "{}")'.format(s1, destination))
